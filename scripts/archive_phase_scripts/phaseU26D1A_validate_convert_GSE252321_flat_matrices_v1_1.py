...
Repair note: v1.1 handles an empty first header field normalized by pandas to ``Unnamed: 0`` and writes reports safely after validation failures.
...
def read_header(path: Path, separator: str) -> List[str]:
    """
    Return the exact column names pandas will use for chunked reads.

    Some deposited matrices have an empty first header field. Pandas
    normalizes this to ``Unnamed: 0``. Using pandas for the header read keeps
    the gene-column name identical between preview, header validation and
    chunked conversion.
    """
    ...
    return list(map(str, header_frame.columns))
...
def convert_one_matrix(
    ...
    qc = {
        "sample_id": sample_id,
        "raw_first_header_name_as_read_by_pandas": header[0],
        "condition": condition,
        "replicate": replicate,
...
def write_report(
    ...
    with path.open("w", encoding="utf-8") as handle:
        ...
        for _, row in qc.iterrows():
            ...
            if validation_error and validation_error.lower() != "nan":
                handle.write(
            ...
            n_cells = int(
                pd.to_numeric(
                    pd.Series([row.get("n_cells", 0)]),
                    errors="coerce",
                ).fillna(0).iloc[0]
            )
            n_genes = int(
                pd.to_numeric(
                    pd.Series([row.get("n_genes", 0)]),
                    errors="coerce",
                ).fillna(0).iloc[0]
            )
            sparsity = float(
                pd.to_numeric(
                    pd.Series([row.get("sparsity_fraction", np.nan)]),
                    errors="coerce",
                ).iloc[0]
            )
            qc_cells = int(
                pd.to_numeric(
                    pd.Series([row.get("adaptive_qc_pass_cells", 0)]),
                    errors="coerce",
                ).fillna(0).iloc[0]
            )
            qc_fraction = float(
                pd.to_numeric(
                    pd.Series(
                        [row.get("adaptive_qc_pass_fraction", np.nan)]
                    ),
                    errors="coerce",
                ).iloc[0]
            )
            ...
            handle.write(
                ...
                f"{n_cells:,} cells, "
                f"{n_genes:,} gene rows, "
                f"sparsity={sparsity_text}, "
                f"scale={row.get('scale_class', 'unresolved')}, "
                f"adaptive QC pass={qc_cells:,}/{n_cells:,} "
                f"({qc_fraction_text}).\n"
            )
        handle.write("\n")
