...
VERSION = "U27B2E1_v1.2_2026-07-16"
...
def normalize_match_text(value: object) -> str:
    """Normalize axis/module text for robust underscore/hyphen-insensitive matching."""
...
def normalize_library(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    library["_axis_norm"] = library["axis"].map(normalize_match_text)
    library["_text"] = (
        library["feature_id"]
        + " "
...
def feature_ids_for_axis(
    ...
    axis_text = (
        library["_axis_norm"]
        if "_axis_norm" in library.columns
        else library["axis"].map(normalize_match_text)
    )
...
def feature_id_column(frame: pd.DataFrame) -> Optional[str]:
    for column in ("feature_id", "submodule_id", "module_id"):
...
def available_ids(frame: pd.DataFrame) -> set[str]:
    column = feature_id_column(frame)
    if column is None:
        return set()
...
def filter_features(
    ...
    subset = frame[
        frame[column].astype(str).isin(
            [str(value) for value in feature_ids]
        )
    ].copy()
    if column != "feature_id" and not subset.empty:
        subset = subset.rename(columns={column: "feature_id"})
    return subset
...
def subtype_support(
    ...
    # Always return the plotting columns, even when the selected pathway has
    # no eligible cell-level rows. This allows the panel renderer to show an
    # explicit no-eligible-values message instead of failing on `_score`.
    for column in ("_label", "_subtype", "_score"):
        if column not in subset.columns:
            subset[column] = pd.Series(index=subset.index, dtype="object")

    if subset.empty:
        subset["_score"] = pd.to_numeric(subset["_score"], errors="coerce")
        return subset

    if "top_refined_subtype" in subset.columns:
        subtype_column = "top_refined_subtype"
    elif "top_population_by_composite_score" in subset.columns:
        subtype_column = "top_population_by_composite_score"
    else:
        subtype_column = None

    if "top_refined_subtype_composite_score" in subset.columns:
        score_column = "top_refined_subtype_composite_score"
    elif "top_population_composite_score" in subset.columns:
        score_column = "top_population_composite_score"
    else:
        score_column = None
...
def axis_feature_sets(
    ...
    return {
        ...
        "nucleotide_nitrogen": feature_ids_for_axis(
            library,
            ["nucleotide nad nitrogen", "nucleotide nitrogen"],
        ),
...
def validate_axis_feature_sets(library: pd.DataFrame) -> pd.DataFrame:
    """Validate the expected nonzero module counts before figure construction."""
    ...
    expected = {
        ...
        "carbon": 11,
        "amino": 8,
        "nucleotide_nitrogen": 9,
        "complement": 9,
    }
    rows = []
    for axis_group, expected_count in expected.items():
        observed = len(set(map(str, sets.get(axis_group, []))))
        rows.append({
            "axis_group": axis_group,
            "expected_modules": expected_count,
            "observed_modules": observed,
            "validation_pass": observed == expected_count,
        })
    audit = pd.DataFrame(rows)
    if not bool(audit["validation_pass"].all()):
        failed = audit.loc[~audit["validation_pass"]].to_dict("records")
        raise RuntimeError(
            "Module-axis mapping validation failed: " + json.dumps(failed)
        )
    return audit
...
def build_pathway_overlap_audit(
    library: pd.DataFrame,
    store,
    full_matrix: pd.DataFrame,
) -> pd.DataFrame:
    sets = axis_feature_sets(library)
    sources = {
        "primary_independent_effects": store.table(
            "Figure_5A", "primary_independent_effects"
        ),
        "broad_effect_reliability": store.table(
            "Figure_5C", "broad_effect_reliability"
        ),
        "module_cellular_synthesis": store.table(
            "Figure_5D", "module_cellular_synthesis"
        ),
        "full_GSE280297_matrix": full_matrix,
    }
    rows = []
    for axis_name, ids in sets.items():
        id_set = set(map(str, ids))
        for source_name, frame in sources.items():
            overlap = sorted(id_set & available_ids(frame))
            rows.append({
                "axis_group": axis_name,
                "source_name": source_name,
                "axis_modules": len(id_set),
                "overlap_modules": len(overlap),
                "overlap_feature_ids": ";".join(overlap),
            })
...
def main() -> int:
    ...
    axis_mapping_audit = validate_axis_feature_sets(module_library)
    ...
    axis_mapping_audit.to_csv(
        outtables
        / "UTI_HostOmics_U27B2E1_axis_mapping_validation.tsv",
        sep="\t",
        index=False,
    )

    overlap_audit = build_pathway_overlap_audit(
        module_library,
        store,
        full_matrix,
    )
    overlap_audit.to_csv(
        outtables
        / "UTI_HostOmics_U27B2E1_pathway_source_overlap_audit.tsv",
        sep="\t",
        index=False,
    )
    log(
        "Pathway/source overlap audit written; cell-level zero-overlap "
        "panels will render explicitly rather than aborting."
    )
    ...
    if (
        ...
        and bool(axis_mapping_audit["validation_pass"].all())
        and bool((overlap_audit["axis_modules"] > 0).all())
    ):
        decision = (
        ...
        [
            {
                ...
                "axis_mapping_validation_pass": bool(
                    axis_mapping_audit["validation_pass"].all()
                ),
                "minimum_axis_modules": int(
                    overlap_audit["axis_modules"].min()
                ),
    ...
    with report_path.open("w", encoding="utf-8") as handle:
        ...
        handle.write(
            f"- Axis mapping validation: "
            f"**{bool(axis_mapping_audit['validation_pass'].all())}**.\n"
        )
        handle.write(
    ...
    log(
        ...
        + ", ".join(
            f"{row.axis_group}={row.observed_modules}"
            for row in axis_mapping_audit.itertuples(index=False)
        )
    )
