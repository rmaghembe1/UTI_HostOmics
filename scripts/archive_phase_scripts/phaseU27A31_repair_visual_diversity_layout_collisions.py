...
VERSION = "U27A31_v1.0_2026-07-15"
...
def ranked_bars(
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(values.index, fontsize=5.45, linespacing=0.95)
    ax.set_xlabel(x_label, fontsize=7.1)
    ax.tick_params(axis="x", labelsize=6.1)
...
def build_figure_7(base, effects, preterm, broad, synthesis, labels, outdir):
    ...
    outputs = save_figure(
        fig, outdir, "UTI_HostOmics_U27A31_Figure_7"
    )
    design = {
...
def build_figure_8(base, effects, preterm, broad, synthesis, labels, outdir):
    ...
    fig.suptitle(FIGURE_TITLES[figure_id], fontsize=10.6, y=0.982)
    outputs = save_figure(
        fig, outdir, "UTI_HostOmics_U27A31_Figure_8"
    )
    design = {
...
def build_figure_9(base, effects, preterm, broad, synthesis, labels, outdir):
    ...
    fig.suptitle(FIGURE_TITLES[figure_id], fontsize=10.6, y=0.982)
    outputs = save_figure(
        fig, outdir, "UTI_HostOmics_U27A31_Figure_9"
    )
    design = {
...
def build_figure_10(base, effects, preterm, broad, synthesis, labels, outdir):
    ...
    outputs = save_figure(
        fig, outdir, "UTI_HostOmics_U27A31_Figure_10"
    )
    design = {
...
def main() -> int:
    ...
    output_manifest.to_csv(
        out_tables / "UTI_HostOmics_U27A31_figure_output_manifest.tsv",
        sep="\t",
        index=False,
    )
    ...
    design_manifest.to_csv(
        out_metadata / "UTI_HostOmics_U27A31_visual_grammar_manifest.tsv",
        sep="\t",
        index=False,
    )
    ...
    phase_decision = (
        "READY_FOR_U27A4_COLLISION_AND_READABILITY_AUDIT"
        if ready
    ...
    pd.DataFrame(
        ...
        out_tables / "UTI_HostOmics_U27A31_phase_decision.tsv",
        sep="\t",
        index=False,
    )

    report_path = out_results / "UTI_HostOmics_U27A31_visual_diversity_report.md"
    with report_path.open("w", encoding="utf-8") as handle:
        handle.write("# Phase U27A.3.1 - Figure visual-diversity redesign\n\n")
    ...
    (out_results / "UTI_HostOmics_U27A31_run_manifest.json").write_text(
