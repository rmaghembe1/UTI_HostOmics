...
VERSION = "U27B2E2C_v1.0_2026-07-16"
...
def log(message: str) -> None:
    print(f"[U27B2E2C] {message}", flush=True)
...
def save_figure(
    ...
    paths: List[Path] = []
    stem = f"UTI_HostOmics_U27B2E2C_Figure_{figure_number}"
...
def save_source_rows(
    ...
    path = (
        tabledir
        / f"UTI_HostOmics_U27B2E2C_Figure_{figure_number}_source_values.tsv"
    )
...
def panel_crops(
    ...
    for index, axis in enumerate(axes):
        ...
        path = (
            outdir
            / f"UTI_HostOmics_U27B2E2C_Figure_{figure_number}"
            f"_panel_{panel}.png"
        )
        crop.save(path)
        paths.append(path)
...
def export_audit(
    ...
    frame.to_csv(
        tabledir / "UTI_HostOmics_U27B2E2C_export_audit.tsv",
        sep="\t",
        index=False,
    )
    return frame
...
def main() -> int:
    ...
    full_contact = (
        outfig
        / "UTI_HostOmics_U27B2E2C_full_figure_contact_sheet.png"
    )
    panel_contact = (
        outfig
        / "UTI_HostOmics_U27B2E2C_panel_contact_sheet.png"
    )
    ...
    build_manifest.to_csv(
        outmetadata
        / "UTI_HostOmics_U27B2E2C_Figures_5_to_8_build_manifest.tsv",
        sep="\t",
        index=False,
    )
    ...
    annotation_audit_frame.to_csv(
        outtables
        / "UTI_HostOmics_U27B2E2C_scatter_annotation_audit.tsv",
        sep="\t",
        index=False,
    )
    ...
    polish_manifest.to_csv(
        outtables
        / "UTI_HostOmics_U27B2E2C_polish_manifest.tsv",
        sep="\t",
        index=False,
    )
    ...
    if (
        ...
        decision = (
            "READY_FOR_U27B2E2D_FINAL_FIGURES_5_TO_8_FREEZE_AUDIT"
        )
    else:
        decision = (
            "TARGETED_U27B2E2C_SCATTER_OR_EXPORT_REPAIR_REQUIRED"
        )
    ...
    pd.DataFrame(
        [
            {
                ...
                "next_phase": (
                    "U27B2E2D final visual freeze audit"
                    if decision.startswith("READY_FOR_U27B2E2D")
        ...
        outtables
        / "UTI_HostOmics_U27B2E2C_phase_decision.tsv",
        sep="\t",
        index=False,
    )
    ...
    report_path = (
        outresults
        / "UTI_HostOmics_U27B2E2C_scatter_annotation_title_repair_report.md"
    )
    with report_path.open("w", encoding="utf-8") as handle:
        handle.write(
            "# Phase U27B2E2C - Final scatter annotation and title repair\n\n"
        )
    ...
    (
        outresults
        / "UTI_HostOmics_U27B2E2C_run_manifest.json"
    ).write_text(
        json.dumps(
            {
                "version": VERSION,
...
if __name__ == "__main__":
    try:
        ...
        print(f"[U27B2E2C] ERROR: {exc}", file=sys.stderr)
        raise
