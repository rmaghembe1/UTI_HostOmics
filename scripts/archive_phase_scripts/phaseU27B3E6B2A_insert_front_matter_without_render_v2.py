...
- Deliberately skips LibreOffice/PDF rendering in this safe split phase.
- Requires a separate external render and page-by-page visual QA phase.
...
VERSION = "U27B3E6B2A_v1.1_2026-07-17"
...
OUTPUT_NAME = (
    ...
    "U27B3E6B2A_front_matter_complete_unrendered.docx"
)
...
def paragraph_index_by_element(doc: Document, paragraph) -> int:
    """Locate a paragraph by its underlying OOXML element.

    python-docx creates new Paragraph wrapper objects on repeated access to
    doc.paragraphs. Comparing wrapper objects with list.index() can therefore
    fail even when both wrappers refer to the same document paragraph.
    """
    target_element = paragraph._p

    for index, candidate in enumerate(doc.paragraphs):
        if candidate._p is target_element:
            return index

    raise RuntimeError(
        "Paragraph OOXML element is not part of the document."
    )


def next_heading_after(doc: Document, paragraph):
    paragraphs = doc.paragraphs
    index = paragraph_index_by_element(doc, paragraph)
...
def remove_block_between(doc: Document, start_paragraph, end_paragraph) -> int:
    paragraphs = doc.paragraphs
    start_index = paragraph_index_by_element(doc, start_paragraph)
    end_index = paragraph_index_by_element(doc, end_paragraph)

    if end_index <= start_index:
        raise RuntimeError(
            "The end heading does not follow the start heading."
        )
...
def main() -> int:
    ...
    # Rendering is intentionally skipped in this safe split phase because the
    # integrated LibreOffice/pdftoppm stage caused the user's WSL instance to
    # terminate. The DOCX is created and structurally audited here; rendering
    # will occur separately in U27B3E6B2B.
    render_result = {
        "render_pass": False,
        "reason": (
            "Intentionally skipped in U27B3E6B2A to avoid WSL termination; "
            "external render and page-by-page visual QA are required."
        ),
        "pdf_path": "",
        "page_pngs": 0,
    }
    contact_sheet = render_dir / "UTI_HostOmics_U27B3E6B2A_render_contact_sheet.png"
    contact_sheet_created = False

    structural_pass = bool(structural_audit["pass"].all())
    render_pass = False
    decision = (
        "READY_FOR_U27B3E6B2B_EXTERNAL_RENDER_AND_VISUAL_QA"
        if structural_pass
        else "TARGETED_U27B3E6B2A_FRONT_MATTER_REPAIR_REQUIRED"
    )

    edit_audit = pd.DataFrame(edit_rows)
    edit_audit.to_csv(
        table_dir / "UTI_HostOmics_U27B3E6B2A_edit_audit.tsv",
        sep="\t",
        index=False,
    )
    text_audit.to_csv(
        table_dir / "UTI_HostOmics_U27B3E6B2A_required_text_audit.tsv",
        sep="\t",
        index=False,
    )
    accession_audit.to_csv(
        table_dir / "UTI_HostOmics_U27B3E6B2A_accession_audit.tsv",
        sep="\t",
        index=False,
    )
    cross_project_audit.to_csv(
        table_dir
        / "UTI_HostOmics_U27B3E6B2A_cross_project_funding_marker_audit.tsv",
        sep="\t",
        index=False,
    )
    media_merge.to_csv(
        table_dir / "UTI_HostOmics_U27B3E6B2A_embedded_media_audit.tsv",
        sep="\t",
        index=False,
    )
    structural_audit.to_csv(
        table_dir / "UTI_HostOmics_U27B3E6B2A_structural_audit.tsv",
        sep="\t",
        index=False,
    )
    ...
    render_audit.to_csv(
        table_dir / "UTI_HostOmics_U27B3E6B2A_render_audit.tsv",
        sep="\t",
        index=False,
    )
    ...
    decision_frame.to_csv(
        table_dir / "UTI_HostOmics_U27B3E6B2A_phase_decision.tsv",
        sep="\t",
        index=False,
    )
    ...
    metadata.to_csv(
        metadata_dir
        / "UTI_HostOmics_U27B3E6B2A_document_provenance.tsv",
        sep="\t",
        index=False,
    )
    ...
    report_path = (
        results_dir
        / "UTI_HostOmics_U27B3E6B2A_front_matter_insertion_report.md"
    )
    report_lines = [
        "# Phase U27B3E6B2A - Safe front-matter insertion without rendering",
        "",
        f"- Version: `{VERSION}`",
        f"- Decision: **{decision}**",
        f"- Source: `{source}`",
        ...
        "- Rendering: **intentionally skipped in this safe split phase**",
        "- Rendered pages: **0**",
        ...
        "## Remaining blockers",
        "",
        "- External rendering and manual visual inspection of every page in U27B3E6B2B.",
    ...
    (
        results_dir / "UTI_HostOmics_U27B3E6B2A_run_manifest.json"
    ).write_text(
    ...
    log("Rendering intentionally skipped to protect WSL stability")
    log(f"Rendered pages: {render_result.get('page_pngs', 0)}")
    log(f"Output DOCX: {output_docx}")
    log("Contact sheet: NOT CREATED IN SAFE SPLIT PHASE")
