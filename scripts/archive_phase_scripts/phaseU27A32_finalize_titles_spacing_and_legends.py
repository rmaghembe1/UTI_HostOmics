...
VERSION = "U27A32_v1.0_2026-07-15"
...
def compact_subtype_series(series: pd.Series) -> pd.Series:
    """Shorten subtype annotations for display without changing values."""
    replacements = {
        "RETNLA MRC1 reparative macrophage": "reparative macrophage",
        "XCR1 CLEC9A cDC1 like": "cDC1-like",
        "CD209A CLEC10A cDC2 like": "cDC2-like",
        "IL2RA TNFRSF18 regulatory type2 like T": "regulatory/type-2-like T",
        "PF4 MRC1 C1Q macrophage": "PF4+ macrophage",
        "C1Q APOE resident macrophage": "resident macrophage",
        "CD83 CLEC10A activated dendritic": "activated dendritic",
        "IL1B CXCL2 inflammatory neutrophil": "inflammatory neutrophil",
        "MKI67 TOP2A cycling immune": "cycling immune",
        "conventional activated T": "activated T",
    }
    output = series.copy()
    labels = []
    for value in output.index.astype(str):
        label = value
...
def attached_colorbar(
    ...
    """Attach a compact colorbar to the owning panel."""
    divider = make_axes_locatable(ax)
    color_axis = divider.append_axes("right", size=size, pad=pad)
    colorbar = ax.figure.colorbar(mappable, cax=color_axis)
    colorbar.set_label(label, fontsize=6.0, labelpad=3)
...
def bubble_matrix(
    ...
    ax.set_title(title, fontsize=8.25, pad=7, loc="left", x=0.03)
    if matrix.empty:
    ...
    scalar.set_array([])
    attached_colorbar(ax, scalar, colorbar_label)
    ax.text(
        0.99,
        0.015,
        "Bubble area proportional to |effect|",
        transform=ax.transAxes,
        ha="right",
        va="bottom",
        fontsize=4.9,
...
def heatmap(
    ...
    clean_axis(ax)
    attached_colorbar(ax, image, colorbar_label)
...
def ranked_bars(
    ...
    ax.set_title(title, fontsize=8.25, pad=7, loc="left", x=0.03)
    series = compact_subtype_series(series)
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(values.index, fontsize=5.20, linespacing=0.92)
    ax.set_xlabel(x_label, fontsize=7.1)
    ax.tick_params(axis="x", labelsize=6.1)
...
def trajectory_plot(
    ...
    palette = plt.get_cmap(cmap)(np.linspace(0.18, 0.88, len(highlighted)))
    ...
    for label, row in numeric.iterrows():
        ...
        if label in highlighted:
            ...
            short_label = short_label.replace(
                "Resistin inflammatory signaling", "Resistin signaling"
            ).replace(
                "Adipokine inflammatory axis", "Adipokine axis"
            ).replace(
                "Lactate-HIF1A glycolysis", "Lactate-HIF1A"
            ).replace(
...
def complement_stage_heatmap(
    ...
    clean_axis(ax)
    attached_colorbar(ax, image, "Effect")
...
def make_layout(figure: str) -> Tuple[plt.Figure, Dict[str, plt.Axes]]:
    if figure == "Figure_7":
        fig = plt.figure(figsize=(JOURNAL_WIDTH_IN, 8.78))
        grid = fig.add_gridspec(
            2, 2, left=0.16, right=0.97, bottom=0.085, top=0.92,
            width_ratios=[1.16, 0.84], height_ratios=[1.0, 1.05],
            wspace=0.84, hspace=0.48,
        )
    elif figure == "Figure_8":
        fig = plt.figure(figsize=(JOURNAL_WIDTH_IN, 8.65))
        grid = fig.add_gridspec(
            2, 2, left=0.16, right=0.97, bottom=0.085, top=0.92,
            width_ratios=[1.0, 1.0], height_ratios=[0.96, 1.04],
            wspace=0.82, hspace=0.50,
        )
    elif figure == "Figure_9":
        fig = plt.figure(figsize=(JOURNAL_WIDTH_IN, 8.92))
        grid = fig.add_gridspec(
            2, 2, left=0.16, right=0.97, bottom=0.085, top=0.92,
            width_ratios=[1.12, 0.88], height_ratios=[0.94, 1.06],
            wspace=0.86, hspace=0.50,
        )
    elif figure == "Figure_10":
        fig = plt.figure(figsize=(JOURNAL_WIDTH_IN, 8.82))
        grid = fig.add_gridspec(
            2, 2, left=0.18, right=0.97, bottom=0.105, top=0.92,
            width_ratios=[1.12, 0.88], height_ratios=[1.0, 1.0],
            wspace=0.86, hspace=0.49,
        )
    else:
...
def build_figure_7(base, effects, preterm, broad, synthesis, labels, outdir):
    ...
    heatmap(
        axes["C"], cellular,
        "UPEC broad-cell log2 fold change",
        cmap="PuOr_r", colorbar_label="log2FC",
    )
    ranked_stems(
        axes["D"], subtype,
        "Top refined-subtype support",
    ...
    outputs = save_figure(
        fig, outdir, "UTI_HostOmics_U27A32_Figure_7"
    )
    design = {
...
def build_figure_8(base, effects, preterm, broad, synthesis, labels, outdir):
    ...
    trajectory_plot(
        axes["C"], cellular,
        "Top broad-cell signaling trajectories",
        cmap="winter", n_highlight=4,
    )
    ranked_bars(
        axes["D"], subtype,
        "Top refined-subtype support",
    ...
    fig.suptitle(FIGURE_TITLES[figure_id], fontsize=10.6, y=0.982)
    outputs = save_figure(
        fig, outdir, "UTI_HostOmics_U27A32_Figure_8"
    )
    design = {
...
def build_figure_9(base, effects, preterm, broad, synthesis, labels, outdir):
    ...
    cleveland_cellular_plot(
        axes["C"], cellular,
        "Broad-cell effect distributions",
    ...
    fig.suptitle(FIGURE_TITLES[figure_id], fontsize=10.6, y=0.982)
    outputs = save_figure(
        fig, outdir, "UTI_HostOmics_U27A32_Figure_9"
    )
    design = {
...
def build_figure_10(base, effects, preterm, broad, synthesis, labels, outdir):
    ...
    complement_stage_heatmap(
        axes["A"], independent,
        "Independent effects by complement stage",
    )
    ...
    outputs = save_figure(
        fig, outdir, "UTI_HostOmics_U27A32_Figure_10"
    )
    design = {
...
def main() -> int:
    ...
    output_manifest.to_csv(
        out_tables / "UTI_HostOmics_U27A32_figure_output_manifest.tsv",
        sep="\t",
        index=False,
    )
    ...
    design_manifest.to_csv(
        out_metadata / "UTI_HostOmics_U27A32_visual_grammar_manifest.tsv",
        sep="\t",
        index=False,
    )
    ...
    phase_decision = (
        "READY_FOR_U27A4_FINAL_VISUAL_AUDIT"
        if ready
    ...
    pd.DataFrame(
        ...
        out_tables / "UTI_HostOmics_U27A32_phase_decision.tsv",
        sep="\t",
        index=False,
    )

    report_path = out_results / "UTI_HostOmics_U27A32_visual_diversity_report.md"
    with report_path.open("w", encoding="utf-8") as handle:
        handle.write("# Phase U27A.3.2 - Figure visual-diversity redesign\n\n")
    ...
    (out_results / "UTI_HostOmics_U27A32_run_manifest.json").write_text(
