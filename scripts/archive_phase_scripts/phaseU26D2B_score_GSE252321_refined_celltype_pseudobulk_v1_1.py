...
Repair note
-----------
Version 1.1 avoids a pandas named-aggregation collision caused by an
output column named ``axis``. No scientific calculations were changed.
...
def module_dictionary(
    ...
    """
    Build the module-to-gene dictionary and one metadata row per module.

    Pandas reserves ``axis`` as a GroupBy.aggregate keyword. A named
    aggregation whose output is also called ``axis`` is therefore parsed as
    the method's axis argument on some pandas versions. Use dictionary-style
    aggregation and rename only the gene-count column afterward.
    """
    required = {
        "feature_id",
        "gene_symbol",
    ...
    missing = sorted(required - set(modules.columns))
    if missing:
        raise RuntimeError(
            f"Canonical module dictionary is missing columns: {missing}"
        )
    ...
    metadata = (
        ...
        .rename(columns={"gene_symbol": "requested_genes"})
    )

    if len(metadata) != len(dictionary):
        raise RuntimeError(
            "Module metadata row count does not match the module dictionary."
        )
